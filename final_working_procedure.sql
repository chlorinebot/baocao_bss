-- Final working procedure với user ID thực tế
USE bc_bss;

-- Xóa procedure cũ
DROP PROCEDURE IF EXISTS GenerateMonthlyScheduleFromRoles;

-- Tạo procedure sử dụng user ID thực tế
DELIMITER $$
...
BEGIN
    ...
    -- Lấy user ID thực tế từ database (user đầu tiên có role_id = 1 - admin)
    ...
    ELSE
        ...
        -- Lưu vào database với user ID thực tế
...
SELECT 'Users hiện tại trong database:' as info;
SELECT id, username, email, firstName, lastName, role_id, isActive FROM users ORDER BY id;
