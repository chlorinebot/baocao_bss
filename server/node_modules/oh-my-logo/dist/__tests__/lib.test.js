...
import { render, renderFilled, resolveColors, DEFAULT_PALETTE, DEFAULT_FONT, DEFAULT_DIRECTION } from '../lib.js';
...
vi.mock('../palettes.js', async () => {
    ...
    return {
        ...actual,
        resolvePalette: vi.fn().mockImplementation((name) => {
            const palettes = {
...
describe('lib', () => {
    ...
    describe('resolveColors', () => {
        ...
        it('should throw error for invalid palette names', () => {
            expect(() => resolveColors('invalid-palette')).toThrow('Unknown palette: invalid-palette');
        });
    ...
    describe('render', () => {
        it('should call renderLogo with default options', async () => {
            await render('TEST');
            expect(renderLogo).toHaveBeenCalledWith('TEST', ['#4ea8ff', '#7f88ff'], DEFAULT_FONT, DEFAULT_DIRECTION);
        });
        it('should call renderLogo with custom options', async () => {
            ...
            expect(renderLogo).toHaveBeenCalledWith('CUSTOM', ['#ff9966', '#ff5e62', '#ffa34e'], 'Big', 'horizontal');
        });
        it('should handle custom color arrays', async () => {
            ...
            expect(renderLogo).toHaveBeenCalledWith('COLORS', customColors, DEFAULT_FONT, 'diagonal');
        });
        ...
        it('should handle partial options', async () => {
            ...
            expect(renderLogo).toHaveBeenCalledWith('PARTIAL', ['#ff9966', '#ff5e62', '#ffa34e'], DEFAULT_FONT, DEFAULT_DIRECTION);
        });
    });
    describe('renderFilled', () => {
        it('should call renderInkLogo with default palette', async () => {
            await renderFilled('TEST');
            expect(renderInkLogo).toHaveBeenCalledWith('TEST', ['#4ea8ff', '#7f88ff']);
        });
        it('should call renderInkLogo with custom palette', async () => {
            ...
            expect(renderInkLogo).toHaveBeenCalledWith('FILLED', ['#ff9966', '#ff5e62', '#ffa34e']);
        });
        it('should handle custom color arrays', async () => {
            const customColors = ['#ff0000', '#00ff00', '#0000ff'];
            const options = {
