-- Fix rotation logic for monthly schedule generation
USE bc_bss;

-- Drop existing procedure
DROP PROCEDURE IF EXISTS GenerateMonthlyScheduleFromRoles;

-- Create updated procedure with correct rotation logic
DELIMITER $$
...
BEGIN
    ...
    WHILE v_current_day <= v_days_in_month DO
        ...
        -- Ngày 2: D(sáng), A(chiều), B(tối), C(nghỉ) - người nghỉ hôm trước làm sáng hôm sau
    ...
    SELECT 
        'success' as status,
        CONCAT('Đã tạo phân công tháng ', p_month, '/', p_year, ' với vai trò ', p_starting_role, ' làm ca sáng ngày đầu (rotation ngược)') as message,
        LAST_INSERT_ID() as schedule_id;
END$$
DELIMITER ;

SELECT 'Rotation logic updated successfully!' as result; 