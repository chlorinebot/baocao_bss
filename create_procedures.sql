-- Tạo stored procedures và functions cho monthly schedule
USE bc_bss;

-- Xóa các procedure/function cũ nếu có
DROP PROCEDURE IF EXISTS GenerateMonthlyScheduleFromRoles;
DROP FUNCTION IF EXISTS GetEmployeeNameByRole;

-- 1. Tạo function GetEmployeeNameByRole
DELIMITER $$
...
-- 2. Tạo procedure GenerateMonthlyScheduleFromRoles
DELIMITER $$
...
BEGIN
    ...
    BEGIN
        ...
        SELECT 'error' as status, CONCAT('SQL Error: ', v_error_msg) as message, 0 as schedule_id;
    END;
    ...
    WHILE v_current_day <= v_days_in_month DO
        -- Calculate roles for current day using rotation logic
        CASE ((v_role_offset + (v_current_day - 1)) % 4)
        ...
        CASE ((v_role_offset + (v_current_day - 1) + 3) % 4)
        ...
        CASE ((v_role_offset + (v_current_day - 1) + 2) % 4)
...
-- 3. Kiểm tra các procedure đã tạo
SELECT 'Procedures và Functions đã được tạo:' as result;
SHOW PROCEDURE STATUS WHERE Name = 'GenerateMonthlyScheduleFromRoles';
SHOW FUNCTION STATUS WHERE Name = 'GetEmployeeNameByRole';

-- 4. Test procedure với tháng 7/2025
